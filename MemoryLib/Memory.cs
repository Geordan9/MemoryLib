...
namespace MemoryLib
{
    public class Memory
    {
        ...
        public byte[] Read(IntPtr MemoryAddress, uint bytesToRead, out int bytesRead)
        {
            ...
            ReadProcessMemory(m_hProcess, MemoryAddress, buffer, bytesToRead, out lpNumberOfBytesRead);
        ...
        public byte[] PointerRead(IntPtr MemoryAddress, uint bytesToRead, int[] Offset, out int bytesRead)
        {
            ...
            {
                IntPtr lpNumberOfBytesRead;
                ReadProcessMemory(m_hProcess, MemoryAddress, buffer1, 4U, out lpNumberOfBytesRead);
                var num3 = ToDec(Make(buffer1)) + Offset[1];
                var buffer2 = new byte[(long) (IntPtr) bytesToRead];
                ReadProcessMemory(m_hProcess, (IntPtr) num3, buffer2, bytesToRead, out lpNumberOfBytesRead);
                bytesRead = lpNumberOfBytesRead.ToInt32();
                return buffer2;
            }
            ...
            for (var index = 0; index <= num1; ++index)
            {
                ...
                if (index == num1)
                {
                    ReadProcessMemory(m_hProcess, (IntPtr) num2, buffer1, 4U, out lpNumberOfBytesRead);
                    bytesRead = lpNumberOfBytesRead.ToInt32();
                    return buffer1;
                }
        ...
        public string PointerWrite(IntPtr MemoryAddress, byte[] bytesToWrite, int[] Offset, out int bytesWritten)
        {
            ...
            {
                IntPtr num2;
                ReadProcessMemory(m_hProcess, MemoryAddress, buffer, 4U, out num2);
                var Decimal2 = ToDec(Make(buffer)) + Offset[1];
                WriteProcessMemory(m_hProcess, (IntPtr) Decimal2, bytesToWrite, (uint) bytesToWrite.Length, out num2);
            ...
            for (var index = 0; index <= num1; ++index)
            {
                ...
                {
                    ReadProcessMemory(m_hProcess, (IntPtr) Decimal1, buffer, 4U, out num2);
                    Decimal1 = ToDec(Make(buffer)) + Offset[index];
                }
            }
